...
public:
	...
	CString					m_name;
	CString					m_dept;
	CString					m_del_project_type;
	char					m_del_project_type_ch[512];
public:
...
//	int CreateAssBlockInspectionInfoTB();	
	int CreateAddMeasurePointinTB();
	int CreateDeleteMeasurePointinTB();
//	int CreateBaseProductionInfoDB();
...
//PM용 테이블 생성관련
	...
	int CreateProjectTB();
	int CreateUserInputBlockListTB();	//-->하남국-20121218
	int CreateVariableOfGroupTB(); //-->하남국-20131113
...
//	int CreateEcoOtsBlockListTB_BlobFiles(CEcoOtsBlockListRecordSet* record, CString user_id);
...
//	int CreateEcoOtsDrawingElementTB_BlobFiles(CEcoOtsDrawingElementRecordSet* record, CString user_id);
...
//	int CreateCameraInfo_BlobFiles(CEcoOtsCameraInfoRecordSet* recordset, CString user_id);
...
//	int CreateProjectionFaceTB_BlobFiles(CEcoOtsProjectionFaceListRecordSet* recordset, CString user_id);
...
//	int CreateProjectionFaceModelingMatrixListTB_BlobFiles(CEcoOtsProjectionFaceModelingMatrixListRecordSet* recordset, CString user_id);
...
public:
	...
	int SelectModel(CString strDB, CString strModelType, CString strWhere, CArray<CEcoModelRecordSet, CEcoModelRecordSet&>* arrModelRS);
...
public:
	...
	int CreateEcoBlockAssemblylevelTB_byCopy();
...
public:
	...
	//MEI tb_ecomodel_???
	int SelectMEIHsfModel(CString project_group, CString ship_no, UINT iNickTable, UINT f_id, CString* strHsfPath);
