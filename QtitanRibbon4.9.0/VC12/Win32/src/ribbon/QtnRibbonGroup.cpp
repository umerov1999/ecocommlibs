...
static QSize imageSize(QStyle* style, const QStyleOptionToolButton& opt, QWidget* widget)
{
    QSize szImage = opt.iconSize;
    if (opt.iconSize == QSize(16, 16))
    {
        static int sz = style->pixelMetric(QStyle::PM_SmallIconSize, &opt, widget);
        szImage = QSize(sz, sz);
    }
    else if (opt.iconSize == QSize(32, 32))
    {
        static int sz = style->pixelMetric(QStyle::PM_LargeIconSize, &opt, widget);
        szImage = QSize(sz, sz);
    }
    return szImage;
}
...
void RibbonGroup::paintEvent(QPaintEvent* event)
{
    ...
    if (isReduced())
    {
        ...
        opt.iconSize = imageSize(style(), opt, this);
