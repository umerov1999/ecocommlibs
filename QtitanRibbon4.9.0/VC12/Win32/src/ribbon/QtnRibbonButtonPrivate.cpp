...
QSize RibbonButton::imageSize(const QStyleOptionToolButton& opt) const
{
    QSize szImage = opt.iconSize;
    if (opt.iconSize == QSize(16, 16))
    {
        const int sz = style()->pixelMetric(QStyle::PM_SmallIconSize, &opt, this);
        szImage = QSize(sz, sz);
    }
