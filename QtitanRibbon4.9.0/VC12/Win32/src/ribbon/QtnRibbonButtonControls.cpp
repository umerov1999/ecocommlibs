...
void RibbonButtonControl::actionChanged()
{
    ...
    int sz = style()->pixelMetric(QStyle::PM_LargeIconSize, &opt, d.m_toolButton);
    const QSize bugImageSize = QSize(sz, sz);
    sz = style()->pixelMetric(QStyle::PM_SmallIconSize, &opt, d.m_toolButton);
    const QSize smallImageSize = QSize(sz, sz);
