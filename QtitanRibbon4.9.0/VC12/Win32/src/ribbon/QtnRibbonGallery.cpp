...
void RibbonGalleryItem::draw(QPainter* p, RibbonGallery* gallery, QRect rectItem, bool enabled, bool selected, bool pressed, bool checked)
{
    ...
    if (OfficeStyle* style = qobject_cast<OfficeStyle*>(gallery->style()) )
    {
        ...
        if (!icon().isNull())
        {
            style->drawRectangle(p, rectItem/*.adjusted(-1, -1, 1, 1)*/, selected, pressed, enabled, checked, false, TypeNormal, BarPopup);
            QPixmap pm = icon().pixmap(rectItem.size(), enabled ? QIcon::Normal : QIcon::Disabled);
            p->save();
            p->setRenderHint(QPainter::SmoothPixmapTransform, true);
            int space = (int)DrawHelpers::dpiScaled(static_cast<qreal>(4));
            p->drawPixmap(rectItem.adjusted(space, space, -space, -space), pm);
            p->restore();
