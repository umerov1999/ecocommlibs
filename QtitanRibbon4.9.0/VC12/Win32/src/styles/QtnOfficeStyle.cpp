...
bool OfficeStyle::drawMenuItem(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt)) 
    {
        ...
        const int iconSize = proxy()->pixelMetric(PM_ToolBarIconSize, opt, widget);
        ...
        if (!menuitem->icon.isNull()) 
        {
            ...
            int iconSize = proxy()->pixelMetric(PM_LargeIconSize, opt, widget);
            ...
            if (checked)
            {
                ...
                drawRectangle(p, rcChecked.adjusted(-2, 0, 2, 0), false/*selected*/, true/*pressed*/, !dis/*enabled*/, 
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        else 
        {
            ...
            if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
            {
                ...
                {
                    pr.setY(isDPIAware() ? int(DrawHelpers::dpiScaled(3.)) : 3);
                    pr.setHeight(toolbutton->iconSize.height() + 3);
                ...
                if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
                {
                    ...
                    QRect rcRowText(QPoint(rcText.left(), rcText.top()), QPoint(rcText.right(), rcText.bottom()));
                    rcRowText.adjust(space, 3, -space, 0);

                    QSize sz = toolbutton->fontMetrics.boundingRect(rcRowText, alignment, toolbutton->text).size();
                    rcRowText.setBottom(rcRowText.top() + sz.height());

                    proxy()->drawItemText(p, rcRowText, alignment, toolbutton->palette, 
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (toolbutton->subControls & SC_ToolButtonMenu) 
        {
            ...
            if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
            {
                ...
                rcArrow = QRect(QPoint(!twoLines ? opt->rect.width() / 2 - 2 : opt->rect.right() - 7, opt->rect.bottom() - heightLine / 2 - 1), QSize(5, 4));
            }
            else
            {
        ...
        else if (toolbutton->features & QStyleOptionToolButton::HasMenu) 
        {
            ...
            if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
            {
                int heightLine = 0;
                bool twoLines = canWordWrap(toolbutton->rect, toolbutton->fontMetrics, toolbutton->text, &heightLine);
                newBtn.rect = QRect(QPoint(!twoLines ? opt->rect.width() / 2 - 2 : opt->rect.right() - 8, opt->rect.bottom() - heightLine/2 - 1), QSize(5, 5));
            }
            else
            {
                QRect ir = menuarea;
...
/*! \internal */
void OfficeStyle::drawItemPixmap(QPainter* painter, const QRect& rect, int alignment, const QPixmap& pixmap) const
{
    if (isDPIAware())
    {
        painter->save();
        QSize szImage = pixmap.size();
        QSize sizeIcon = QSize(int(DrawHelpers::dpiScaled(szImage.width())), int(DrawHelpers::dpiScaled(szImage.height())));
        if (sizeIcon.width() > rect.width())
            sizeIcon = szImage;
        szImage = sizeIcon.isValid() && !sizeIcon.isNull() ? sizeIcon : pixmap.size();
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
        qreal scale = pixmap.devicePixelRatio();
#else
        qreal scale = 1.0;
#endif
        QRect aligned = alignedRect(QApplication::layoutDirection(), QFlag(alignment), szImage / scale, rect);
        QRect inter = aligned.intersected(rect);
        painter->setRenderHint(QPainter::SmoothPixmapTransform, true);
        painter->drawPixmap(inter.x(), inter.y(), sizeIcon.width(), sizeIcon.height(), pixmap);
        painter->restore();
    }
    else
        CommonStyle::drawItemPixmap(painter, rect, alignment, pixmap);
}
