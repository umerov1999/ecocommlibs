                    ...
                    if ((tb->features & (QStyleOptionToolButton::MenuButtonPopup | QStyleOptionToolButton::PopupDelay)) == QStyleOptionToolButton::MenuButtonPopup)
                    {
                        if (!qobject_cast<const RibbonGroupPopupButton *>(widget)) 
                        {
                            if (tb->toolButtonStyle == Qt::ToolButtonTextUnderIcon) 
                            {
                                ...
                                QSize textSize = tb->fontMetrics.boundingRect(opt->rect, alignment, tb->text).size();
    ...
    if (qobject_cast<const RibbonBackstageButton*>(w))
    {
        if (const StyleOptionBackstageButton* toolbutton = qstyleoption_cast<const StyleOptionBackstageButton*>(opt))
        {
            ...
            if (((!hasArrow && toolbutton->icon.isNull()) && !toolbutton->text.isEmpty())|| toolbutton->toolButtonStyle == Qt::ToolButtonTextOnly) 
            {
                ...
                proxy()->drawItemText(p, rect, alignment, toolbutton->palette, /*opt->state & State_Enabled*/true, 
            ...
            else
            {
                ...
                if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
                {
                    ...
                    if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
                    {
                        ...
                        const int space = 1;//toolbutton->fontMetrics.width(QLatin1Char(' '));
        ...
        if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
        {
            ...
            else if (toolbutton->features & QStyleOptionToolButton::HasMenu) 
            {
                ...
                if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
                {
                    ...
                    QRect rcRowText(QPoint(rc.left(), rc.top()), QPoint(rc.right(), rc.bottom()));
                    rcRowText.setY((isDPIAware() ? int(DrawHelpers::dpiScaled(3.)) : 3) + toolbutton->iconSize.height() + 3);
                    QSize sz = toolbutton->fontMetrics.boundingRect(newBtn.rect, alignment, toolbutton->text).size();
                    rcRowText.setBottom((rcRowText.top() + sz.height()) - (toolbutton->fontMetrics.height() - toolbutton->fontMetrics.descent()*2));
                    QRect r = rcRowText;
                    newBtn.rect = QRect(QPoint(r.right() - 8, rcRowText.bottom()), QSize(5, 5));
                }
                else
                {
                    QRect ir = menuarea;
...
static RibbonBar* findMainWindow()
{
    foreach(QWidget *widget, qApp->topLevelWidgets())
    {
        if (QMainWindow* mainWindow = qobject_cast<QMainWindow *>(widget))
        {
            ...
            {
                QList<RibbonBar*> l = mainWindow->findChildren<RibbonBar*>();
                if (l.size() > 0)
                    return l.at(0);
            }
        }
