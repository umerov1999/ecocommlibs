...
void RibbonPaintManager2013::drawRibbonBackstageCloseButton(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolButton* optButton = qstyleoption_cast<const QStyleOptionToolButton*>(opt)) 
    {
        if (!optButton->icon.isNull())
        {
            ...
            QPixmap pxIcon = optButton->icon.pixmap(optButton->iconSize, enabled ? (highlighted ? QIcon::Selected : QIcon::Active) : QIcon::Disabled, QIcon::On);
            p->save();
            p->setRenderHint(QPainter::SmoothPixmapTransform, true);
            p->drawPixmap(QRect(QPoint(), optButton->iconSize), pxIcon);
            p->restore();
        }
    }
}
