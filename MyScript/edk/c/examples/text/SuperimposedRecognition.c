/**
 * The `SuperimposedRecognition` example code shows you how to perform
 * handwriting recognition on superimposed digital ink.
 *
 * Performing a recognition with MyScript Superimposed is the same as 
 * performing a recognition with MyScript Text. Workflows are similar. 
 * Only the AK resources differ. MyScript Superimposed uses dedicated 
 * resources and processes superimposed text samples (characters written
 * one over the others).
 */
...
{
  const char* default_argv[] = { argv[0], DATA_DIR "/hello_world_superimposed.ink" };
  voStructuredInputRecognizer recognizer = NULL;
  voResource akSuperimposed = NULL;
  ...
  akSuperimposed = loadEngineObject(AK_SUPERIMPOSED_RES);
  printf(" . SuperimposedAlphabetKnowledge resource loaded successfully\n");
  verify(VO_IFACE(engine, IAttachTarget)->attach(engine, recognizer, akSuperimposed));
  printf(" . SuperimposedAlphabetKnowledge resource attached successfully\n");
  ...
  verify(voReleaseObject(engine, akSuperimposed));
  ...
  printf(" you just learnt how to perform handwriting recognition on superimposed digital ink \n");
