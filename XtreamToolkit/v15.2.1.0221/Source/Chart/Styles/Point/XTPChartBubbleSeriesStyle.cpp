...
CXTPChartDeviceCommand* CXTPChartBubbleSeriesPointView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	if (pStyle->GetMinSize() < CXTPChartMathUtils::m_dEPS && pSeriesView->m_dMaxValue > CXTPChartMathUtils::m_dEPS)
	{
		dSize = dValue * pStyle->GetMaxSize() / pSeriesView->m_dMaxValue;
	}
	else if (fabs(pSeriesView->m_dMaxValue - pSeriesView->m_dMinValue) < CXTPChartMathUtils::m_dEPS)
	{
	...
	if (fabs(dSize) < CXTPChartMathUtils::m_dEPS) dSize = 0.5;
