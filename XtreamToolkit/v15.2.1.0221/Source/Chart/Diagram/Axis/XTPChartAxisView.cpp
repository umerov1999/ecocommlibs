...
double CXTPChartAxisView::ValueToAxis(double dValue) const
{
	...
	if (fabs(dValue) < CXTPChartMathUtils::m_dEPS)
		return 0;
...
BOOL CXTPChartDiagram2DAxisView::IsScollBarVisible() const
{
	...
	if (fabs(GetViewRangeMinValue() - GetRangeMinValue()) < CXTPChartMathUtils::m_dEPS &&
		fabs(GetViewRangeMaxValue() - GetRangeMaxValue()) < CXTPChartMathUtils::m_dEPS)
		return FALSE;
...
double CXTPChartAxisView::CalculateGridSpacing(double nAxisRangeDelta, double nScreenDelta, double nGridSpacingFactor)
{
	...
	if (deltaCoef < CXTPChartMathUtils::m_dEPS)
		return 1;
...
void CXTPChartDiagram2DAxisView::CreateTickMarks(CXTPChartDeviceContext* pDC, CRect rcDiagram)
{
	...
	{
		if (dAxisMinValue < -CXTPChartMathUtils::m_dEPS)
			dAxisMinValue += -CXTPChartMathUtils::m_dEPS;
