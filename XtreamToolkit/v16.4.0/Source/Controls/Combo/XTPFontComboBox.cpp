...
void CXTPFontComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	// draw background.
	CXTPComboBoxTheme* pTheme = GetTheme();
	ASSERT(NULL != pTheme);

	if (bSelected)
	{
		pDC->SetTextColor(GetXtremeColor(XPCOLOR_HIGHLIGHT_TEXT));
		pDC->FillSolidRect(&rcItem, GetXtremeColor(XPCOLOR_HIGHLIGHT));
	}
	else
	{
		...
		CBrush* pBrush = CBrush::FromHandle(GetClientBrush(pDC));
		pDC->FillRect(&rcItem, pBrush);
