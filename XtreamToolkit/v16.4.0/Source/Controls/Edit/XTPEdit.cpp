...
CXTPEdit::CXTPEdit()
{
	...
	m_clrReadonlyText = COLORREF_NULL;
}
...
XTPControlTheme CXTPEdit::GetTheme()
{
	return m_nTheme;
}
...
void CXTPEdit::RefreshMetrics()
{
	...
	if (m_nTheme == xtpControlThemeResource)
	{
		...
		m_clrBorderNormal = pImages->GetImageColor(_T("Toolbar"), _T("ControlEditBorder"));
		m_clrBorderHot    = pImages->GetImageColor(_T("Toolbar"), _T("ControlEditBorder"));
	}

	if (xtpControlThemeOffice2013 == m_nTheme)
	{
		m_clrBorderNormal = RGB(171,171,171);
		m_clrBorderHot = RGB(163,189,227);
		m_clrReadonlyText = RGB(255, 0, 0);

		m_clrText = RGB(68, 68, 68);
		m_clrBackground = RGB(255, 255, 255);
	}

	if (xtpControlThemeVisualStudio2012 == m_nTheme)
	{
		CXTPResourceImages *pImages = XTPResourceImages();
		LPCTSTR lpszKey = _T("Controls.Edit");
		m_clrBorderNormal = pImages->GetImageColor(lpszKey, _T("BorderNormal"));
		m_clrBorderHot    = m_clrBorderNormal; //pImages->GetImageColor(lpszKey, _T("BorderHot"));
		m_clrBackground   = pImages->GetImageColor(lpszKey, _T("Background"));
		m_clrText         = pImages->GetImageColor(lpszKey, _T("Text"));
		m_clrReadonlyText = pImages->GetImageColor(lpszKey, _T("TextReadOnly"));

		m_brBackground.DeleteObject();
		m_brBackground.CreateSolidBrush(m_clrBackground);
	}
	else if (m_nTheme == xtpControlThemeVisualStudio2012Light)
	{
		m_clrBorderNormal = RGB(230, 231, 232);
		m_clrBorderHot    = RGB(230, 231, 232);
		m_clrBackground   = RGB(230, 231, 232);
		m_clrText         = RGB( 30,  30,  30);
		m_clrReadonlyText = RGB(113, 113, 113);

		m_brBackground.DeleteObject();
		m_brBackground.CreateSolidBrush(m_clrBackground);
	}
	else if (m_nTheme == xtpControlThemeVisualStudio2012Dark)
	{
		m_clrBorderNormal = RGB( 37,  37,  38);
		m_clrBorderHot    = RGB( 37,  37,  38);
		m_clrBackground   = RGB( 37,  37,  38);
		m_clrText         = RGB(241, 241, 241);
		m_clrReadonlyText = RGB(153, 153, 153);
...
{
	if ((m_nTheme != xtpControlThemeVisualStudio2012Dark) &&
		(m_nTheme != xtpControlThemeVisualStudio2012Light)) return NULL;

	switch (nCtlColor)
	{
	case CTLCOLOR_STATIC://when edit is disabled or read only
		{
			pDC->SetTextColor(m_clrReadonlyText);//only when edit is read only
...
void CXTPEditor::RefreshMetrics()
{
	...
	{
		CXTPResourceImages *pImages = XTPResourceImages();
		LPCTSTR lpszKey = _T("Controls.Editor");
		m_clrBorderNormal = pImages->GetImageColor(lpszKey, _T("BorderNormal"));
		m_clrBorderHot    = pImages->GetImageColor(lpszKey, _T("BorderHot"));
		m_clrBackground   = pImages->GetImageColor(lpszKey, _T("Background"));
		m_clrText         = pImages->GetImageColor(lpszKey, _T("Text"));
		m_clrReadonlyText = pImages->GetImageColor(lpszKey, _T("TextReadOnly"));
	...
	if (xtpControlThemeVisualStudio2012Light == m_nTheme)
	{
		m_clrBorderNormal = RGB(230, 231, 232);
		m_clrBorderHot    = RGB(230, 231, 232);
		m_clrBackground   = RGB(230, 231, 232);
	...
	if (xtpControlThemeVisualStudio2012Dark == m_nTheme)
	{
		m_clrBorderNormal = RGB( 37,  37,  38);
		m_clrBorderHot    = RGB( 37,  37,  38);
		m_clrBackground   = RGB( 37,  37,  38);
