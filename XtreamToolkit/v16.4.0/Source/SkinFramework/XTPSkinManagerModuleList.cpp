...
public:
	...
	virtual BOOL GetModuleInformation(HMODULE hModule, XTP_MODULEINFO* lpmodinfo) = 0;

protected:
	int m_dwProcessId;
	HMODULE m_hLockedModule;

	HMODULE LockModule(HMODULE hModule);
	void ReleaseLockedModule();
};

CXTPSkinManagerModuleList::CModuleEnumerator::CModuleEnumerator(DWORD dwProcessId)
	: m_dwProcessId(dwProcessId)
	, m_hLockedModule(NULL)
{
...
HMODULE CXTPSkinManagerModuleList::CModuleEnumerator::LockModule(HMODULE hModule)
{
	ReleaseLockedModule();

	m_hLockedModule = hModule;
	if(NULL != m_hLockedModule)
	{
		TCHAR szModulePath[MAX_PATH + 1];
		if(0 < GetModuleFileName(m_hLockedModule, szModulePath, MAX_PATH))
		{
			m_hLockedModule = LoadLibrary(szModulePath);
		}
		else
		{
			m_hLockedModule = NULL;
	...
	return m_hLockedModule;
}

void CXTPSkinManagerModuleList::CModuleEnumerator::ReleaseLockedModule()
{
	if(m_hLockedModule)
	{
		::FreeLibrary(m_hLockedModule);
		m_hLockedModule = NULL;
	}
}
...
BOOL CXTPSkinManagerModuleList::CPsapiModuleEnumerator::GetModuleInformation(HMODULE hModule, XTP_MODULEINFO* lpmodinfo)
{
	...
	if (!m_pfnGetModuleInformation(hProcess, hModule, lpmodinfo, sizeof(XTP_MODULEINFO)))
	{
...
HMODULE CXTPSkinManagerModuleList::CPsapiModuleEnumerator::GetFirstModule()
{
	...
	return LockModule(m_pModules[0]);
}
...
HMODULE CXTPSkinManagerModuleList::CPsapiModuleEnumerator::GetNextModule()
{
	...
	return LockModule(m_pModules[m_nIndex]);
}
...
HMODULE CXTPSkinManagerModuleList::CToolHelpModuleEnumerator::GetFirstModule()
{
	...
	{
		return LockModule(m_me.hModule);
	}
...
BOOL CXTPSkinManagerModuleList::GetModuleInformation(HMODULE hModule, XTP_MODULEINFO* lpmodinfo)
{
	...
	if (m_pEnumerator)
	{
		return m_pEnumerator->GetModuleInformation(hModule, lpmodinfo);
	}
