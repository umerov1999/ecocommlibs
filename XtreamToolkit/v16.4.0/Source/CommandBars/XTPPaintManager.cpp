...
void CXTPPaintManager::RefreshMetrics()
{
	...
	m_clrStatusTextGrayColor = GetXtremeColor(COLOR_GRAYTEXT);
	m_clrStatusGripperLite = GetXtremeColor(COLOR_BTNHIGHLIGHT);
	m_clrStatusGripperDark = GetXtremeColor(COLOR_3DSHADOW);

	// Message bar
	m_rcMessageBarMargin.SetRect(3, 3, 3, 3);
...
#ifdef _XTP_INCLUDE_RIBBON
		case xtpThemeRibbon:
			//if (!XTPResourceImages()->IsValid() || XTPResourceImages()->m_bLoadedNothing)
			if (XTPResourceImages()->m_bLoadedNothing)
			{
...
/*
	if (bAlert)
	{
		AfxMessageBox(_T("Could not locate valid resources for this theme.\n\n"
			"Make sure you have loaded the resource DLL prior to setting the theme for the control.\n\n"
...
*/
	...
	switch(paintTheme)
	{
		...
		case xtpThemeResource:
		{
			if (_T("") != sCompatibility)
			{
				XTPResourceImages()->SetHandle(NULL, NULL, TRUE);
			}
...
{
	CXTPPaintManager* pOldTheme = s_pInstance;
	s_pInstance = pTheme;
...
void CXTPPaintManager::Rectangle(CDC* pDC, LPCRECT rc, int nPen, int nBrush)
{
...
void CXTPPaintManager::Line(CDC* pDC, int x0, int y0, int x1, int y1, int nPen)
{
	if (x0 == x1)
	{
		VerticalLine(pDC, x0, y0, y1, GetXtremeColor(nPen));
	}
	else if (y0 == y1)
	{
		HorizontalLine(pDC, x0, y0, x1, GetXtremeColor(nPen));
	}
	else
	{
...
CSize CXTPPaintManager::DrawControlText2(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bVert, BOOL bTriangled)
{
	...
	if (!bDraw)
	{
		...
		CSize sz(max(szFirstRow.cx, szSecondRow.cx), !bVert ? szFirstRow.cy * 2 : 24);
	...
	if (!bVert)
	{
		...
		if (bTriangled && strSecondRow.IsEmpty())
		{
			rcFirstRowText.OffsetRect(0, XTP_COMMANDBAR_TRIANGLE_SIZE / 2);
		}
...
CSize CXTPPaintManager::DrawControlText(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bVert, BOOL bCentered, BOOL bTriangled)
{
	...
	if (!bDraw)
	{
		...
		if (bTriangled) sz.cx += m_bOffice2007Padding ? XTP_COMMANDBAR_TRIANGLE_SIZE : XTP_COMMANDBAR_TRIANGLE_SIZE + 2;
		return bVert ? CSize(sz.cy, sz.cx) : sz;
	}
...
void CXTPPaintManager::UpdateFonts()
{
	...
	if (m_bUseStandardFont)
	{
		...
		m_nTextHeight = CalcTextHeight(lf.lfHeight);
		m_nEditHeight = CalcCtrlHeight(lf.lfHeight);;
...
void CXTPPaintManager::DrawHiddenGlyph(CDC* pDC, CXTPControl* pButton, COLORREF clr, BOOL bVertical)
{
	...
	{
		CXTPPenDC pen(*pDC, clr);
		pDC->MoveTo(rcButton.right - 8, rcButton.top + 2);
		pDC->LineTo(rcButton.right - 6, rcButton.top + 4);
		pDC->LineTo(rcButton.right - 3, rcButton.top + 1);

		pDC->MoveTo(rcButton.right - 8, rcButton.top + 3);
		pDC->LineTo(rcButton.right - 6, rcButton.top + 5);
		pDC->LineTo(rcButton.right - 3, rcButton.top + 2);

		pDC->MoveTo(rcButton.right - 8, rcButton.top + 6);
		pDC->LineTo(rcButton.right - 6, rcButton.top + 8);
		pDC->LineTo(rcButton.right - 3, rcButton.top + 5);

		pDC->MoveTo(rcButton.right - 8, rcButton.top + 7);
		pDC->LineTo(rcButton.right - 6, rcButton.top + 9);
		pDC->LineTo(rcButton.right - 3, rcButton.top + 6);
	}
	else
	{
		CXTPPenDC pen(*pDC, clr);
		pDC->MoveTo(rcButton.left + 2, rcButton.top + 4);
		pDC->LineTo(rcButton.left + 4, rcButton.top + 6);
		pDC->LineTo(rcButton.left + 1, rcButton.top + 9);

		pDC->MoveTo(rcButton.left + 3, rcButton.top + 4);
		pDC->LineTo(rcButton.left + 5, rcButton.top + 6);
		pDC->LineTo(rcButton.left + 2, rcButton.top + 9);

		pDC->MoveTo(rcButton.left + 6, rcButton.top + 4);
		pDC->LineTo(rcButton.left + 8, rcButton.top + 6);
		pDC->LineTo(rcButton.left + 5, rcButton.top + 9);

		pDC->MoveTo(rcButton.left + 7, rcButton.top + 4);
		pDC->LineTo(rcButton.left + 9, rcButton.top + 6);
		pDC->LineTo(rcButton.left + 6, rcButton.top + 9);
	}
}
...
CSize CXTPPaintManager::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
	if (controlType == xtpButtonExpandToolbar)
	{
		...
		else
		{
			...
			if (!IsVerticalPosition(pBar->GetPosition()))
			{
				...
				DrawExpandGlyph(pDC, pButton, CPoint(pt.x -2 , pt.y - 1), CPoint(pt.x + 2, pt.y - 1), CPoint (pt.x, pt.y + 1), clrText);
				if (bHiddenExists)
				{
					DrawHiddenGlyph(pDC, pButton, clrText, TRUE);
				}
			}
			else
			{
				CPoint pt = CPoint(rcButton.left + 4, rcButton.top + 5);
				DrawExpandGlyph(pDC, pButton, CPoint(pt.x +2 , pt.y - 2), CPoint(pt.x + 2, pt.y + 2), CPoint (pt.x, pt.y), clrText);

				if (bHiddenExists)
				{
					DrawHiddenGlyph(pDC, pButton, clrText, FALSE);
				}
			}
		}
	}
	...
	{
		CFont font;
		font.CreatePointFont(MulDiv(80, XTP_DEFAULT_DISPLAY_DPI, pDC->GetDeviceCaps(LOGPIXELSX)), _T("Marlett"));
...
CSize CXTPPaintManager::DrawControlPopupParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		if (controlType == xtpControlButton && pButton->GetCustomShortcutText().GetLength() != 0)
			rcText.right -= pDC->GetTextExtent(pButton->GetCustomShortcutText()).cx + 19;
...
int CXTPPaintManager::CalcEntityHeight(
	int nFontHeight,
	int nFactor,
...
int CXTPPaintManager::CalcTextHeight(
	int nFontHeight,
...
int CXTPPaintManager::CalcCtrlHeight(
	int nFontHeight,
...
int CXTPPaintManager::CalcSplitButtonDropDownWidth(int nTextHeight)
{
	nTextHeight = abs(nTextHeight);
	return max(XTP_COMMANDBAR_SPLITBUTTON_DROPDOWN_WIDTH, nTextHeight / 2);
}

int CXTPPaintManager::CalcSplitButtonDropDownHeight(int nFontHeight)
{
...
BOOL IsDPIIconsScallingEnabled(CXTPControl *pButton)
{
...
CSize CXTPPaintManager::DrawControlToolBarParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	BOOL bDPIIconsScallingEnabled = IsDPIIconsScallingEnabled(pButton);
	const int nDpiScallingButtonMargin = 4;
	...
	if (buttonStyle == xtpButtonIconAndCaptionBelow)
	{
		...
		if (bDraw)
		{
			if (bDrawImage)
			{
				CSize szScalledIcon = (!bDPIIconsScallingEnabled) ? szStretchIcon :
					CSize(XTPDpiHelper()->ScaleX(szStretchIcon.cx), XTPDpiHelper()->ScaleY(szStretchIcon.cy));
				...
				CPoint pt = bVert ? CPoint(rcButton.left + 4 + nSplitDropDownHeight, rcButton.CenterPoint().y - szScalledIcon.cy / 2) :
					CPoint(rcButton.CenterPoint().x - szScalledIcon.cx / 2, rcButton.top + 3);
	...
	if (bDraw && pImage)
	{
		...
		if (bDPIIconsScallingEnabled)
		{
			szIcon.cx = min(szButton.cx - nDpiScallingButtonMargin, szIcon.cx);
			szIcon.cy = min(szButton.cy - nDpiScallingButtonMargin, szIcon.cy);
		}
...
BEGIN_DISPATCH_MAP(CXTPPaintManager, CCmdTarget)
...
// {555113E7-46A4-4f99-83D8-C6D3C2B390FC}
static const IID IID_IPaintManager =
{ 0x555113e7, 0x46a4, 0x4f99, { 0x83, 0xd8, 0xc6, 0xd3, 0xc2, 0xb3, 0x90, 0xfc } };


IMPLEMENT_OLETYPELIB_EX(CXTPPaintManager, IID_IPaintManager)


BEGIN_INTERFACE_MAP(CXTPPaintManager, CCmdTarget)
	INTERFACE_PART(CXTPPaintManager, IID_IPaintManager, Dispatch)
END_INTERFACE_MAP()
