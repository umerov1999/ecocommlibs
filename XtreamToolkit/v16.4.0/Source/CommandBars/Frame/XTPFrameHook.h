...
private:
	...
	static int GetMouseTrackingAllowance();
	static void CALLBACK MouseTrackingTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime);
	static void CALLBACK MouseUnlockingTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime);
	BOOL GlueMouseToTheRightBorder(int nAllowance, CPoint ptCursor, const CRect& rcWnd, const CRect& rcBorder);
	BOOL GlueMouseToTheLeftBorder(int nAllowance, CPoint ptCursor, const CRect& rcWnd, const CRect& rcBorder);
	BOOL GlueMouseToTheTopBorder(int nAllowance, CPoint ptCursor, const CRect& rcWnd, const CRect& rcBorder);
	BOOL GlueMouseToTheBottomBorder(int nAllowance, CPoint ptCursor, const CRect& rcWnd, const CRect& rcBorder);
	BOOL LockMouseOnBorder(CPoint ptCursor, CRect rcBorderLine);

	UINT m_nMouseBorderCaptureCounter;
	UINT_PTR m_nMouseTrackingTimerId;

	struct MOUSE_TRACKING_DATA : public CNoTrackObject
	{
		CRect rcBorder;
		CMap<UINT_PTR, UINT_PTR, CXTPCommandBarsFrameHook*, CXTPCommandBarsFrameHook*> TimerIdToFrameMap;
	};

	static CThreadLocal<MOUSE_TRACKING_DATA> m_ThreadMouseTrackingData;
