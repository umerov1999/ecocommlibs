...
BEGIN_DISPATCH_MAP(CXTPReportDataManager, CCmdTarget)
...
// {124283D9-27ED-44d7-970B-9F526E977C3C}
static const GUID IID_IReportDataManager =
{ 0x124283d9, 0x27ed, 0x44d7, { 0x97, 0xb, 0x9f, 0x52, 0x6e, 0x97, 0x7c, 0x3c } };


BEGIN_INTERFACE_MAP(CXTPReportDataManager, CCmdTarget)
	INTERFACE_PART(CXTPReportDataManager, IID_IReportDataManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportDataManager, IID_IReportDataManager)
...
BOOL CXTPReportDataManager::OleUpdateField(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecordItemVariant* pItem = DYNAMIC_DOWNCAST(CXTPReportRecordItemVariant, CXTPReportRecordItemVariant::FromIDispatch(lpDispatch));
	if (!pItem)
		return FALSE;
...
BOOL CXTPReportDataManager::OleUpdateRecordItem(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecordItem* pRecordItem = DYNAMIC_DOWNCAST(CXTPReportRecordItem, CCmdTarget::FromIDispatch(lpDispatch));
	if (!pRecordItem)
		return FALSE;
