	...
	for (int nColumn = 0; nColumn < nColumnCount; nColumn++)
	{
		...
		if (pColumn && pColumn->IsVisible())
		{
			...
			if (bAsMerged && pItem->IsMerged())
			{
				...
				CXTPReportColumn* pRightmostColumn = pColumns->GetAt(nLeftmostColumn);
	...
	XTP_REPORTRECORDITEM_METRICS  *pItemMetrics)
{
		...
		&& !pDrawArgs->pControl->IsIconView())
	{
		...
		{
			pItemMetrics->clrForeground = (lf.lfWeight == FW_BOLD) ? pPaintManager->m_clrBoldTextHighlight : pPaintManager->m_clrHighlightText;
...
BEGIN_DISPATCH_MAP(CXTPReportRow, CCmdTarget)
...
// {B5989CE5-E015-4f87-885D-DE3326A63BEA}
static const GUID IID_IReportRow =
{ 0xB5989ce5, 0xe015, 0x4f87, { 0x88, 0x5d, 0xde, 0x33, 0x26, 0xa6, 0x3b, 0xea } };


IMPLEMENT_OLETYPELIB_EX(CXTPReportRow, IID_IReportRow)
...
void CXTPReportRow::OleGetRect(long* Left, long* Top, long* Right, long* Bottom)
{
	...
	CXTPReportScreenRows *pScreenRows = m_pControl->GetRows()->GetScreenRows();
...
{
	CXTPReportRecordItem* pItem = DYNAMIC_DOWNCAST(CXTPReportRecordItem, CXTPReportRecordItem::FromIDispatch(lpDispatch));
	if (!pItem)
		return;
...
#ifdef _XTP_ACTIVEX
	INTERFACE_PART(CXTPReportRow, IID_IReportRow, Dispatch)
#endif
