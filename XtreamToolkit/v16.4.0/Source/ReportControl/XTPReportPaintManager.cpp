...
void CXTPReportPaintManager::RefreshMetrics()
{
	...
	m_clrBoldText.SetStandardValue(GetXtremeColor(COLOR_WINDOWTEXT));
	m_clrBoldTextHighlight.SetStandardValue(GetXtremeColor(COLOR_HIGHLIGHTTEXT));
...
CRect CXTPReportPaintManager::GetGroupRowBmpSize(CRect rcRow, int nBitmapOffset)
{
	...
	rcBitmap.OffsetRect(nBitmapOffset, (m_szGlyph.cy / 2) - 1);
	return rcBitmap;
}
		...
		while (pos > - 1)
		{
			...
			if (pos > - 1)
			{
				...
				if (pColumn && pColumn->IsVisible())
				{
					...
					{
						CXTPReportColumn* pColumn = pControl->GetColumns()->Find(i);
						if (NULL != pColumn)
						{
							if (IsColumnWidthWYSIWYG())
							{
								nColPrintWidth += pColumn->GetWidth();
							}
							else
							{
								nColPrintWidth += pColumn->GetPrintWidth(rcRow.Width());
							}
						}
					}
...
int CXTPReportPaintManager::GetRowHeightEx(CDC *pDC, CXTPReportRow *pRow, int nTotalWidth)
{
...
void CXTPReportPaintManager::DrawColumnFooterDivider(CDC* pDC, CXTPReportColumn* pColumn, CRect rcColumn)
{
	if (pColumn->GetDrawFooterDivider())
	{
		...
		DrawVerticalLine(pDC, rcColumn.right - 1, rcColumn.top + 5, rcColumn.Height() - 9, m_clrControlDark);
	}
}
...
// {0FA678BA-50F7-4163-9DA2-593CB46A394F}
static const GUID IID_IReportHyperlinkStyle =
{ 0xfa678ba, 0x50f7, 0x4163, { 0x9d, 0xa2, 0x59, 0x3c, 0xb4, 0x6a, 0x39, 0x4f } };

BEGIN_INTERFACE_MAP(CXTPReportHyperlinkStyle, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportHyperlinkStyle, IID_IReportHyperlinkStyle, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportHyperlinkStyle, IID_IReportHyperlinkStyle)
...
BEGIN_DISPATCH_MAP(CXTPReportPaintManager, CCmdTarget)
...
// {A5989CE5-E015-4f87-885D-DE3326A63BEA}
static const GUID IID_IReportPaintManager =
{ 0xA5989ce5, 0xe015, 0x4f87, { 0x88, 0x5d, 0xde, 0x33, 0x26, 0xa6, 0x3b, 0xea } };

BEGIN_INTERFACE_MAP(CXTPReportPaintManager, CCmdTarget)
	INTERFACE_PART(CXTPReportPaintManager, IID_IReportPaintManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportPaintManager, IID_IReportPaintManager)
...
{
	return AxCreateOleFont(&m_fontText, this, (LPFNFONTCHANGED)&CXTPReportPaintManager::OleSetTextFont);
}
...
{
	return AxCreateOleFont(&m_fontPreview, this, (LPFNFONTCHANGED)&CXTPReportPaintManager::OleSetPreviewTextFont);
}

void CXTPReportPaintManager::OleSetPreviewTextFont(LPFONTDISP pFontDisp)
{
	AxCreateFontFromDispatch(&m_fontPreview, pFontDisp);
}

LPFONTDISP CXTPReportPaintManager::OleGetCaptionFont()
{
	return AxCreateOleFont(&m_fontCaption, this, (LPFNFONTCHANGED)&CXTPReportPaintManager::OleSetCaptionFont);
}
