...
BEGIN_DISPATCH_MAP(XTP_DAYITEM_METRICS, CCmdTarget)
...
// {236C965B-40BE-4ce2-BE64-AA743BBE44BD}
static const GUID IID_IDatePickerDayMetrics =
{ 0x236c965b, 0x40be, 0x4ce2, { 0xbe, 0x64, 0xaa, 0x74, 0x3b, 0xbe, 0x44, 0xbd } };

BEGIN_INTERFACE_MAP(XTP_DAYITEM_METRICS, CCmdTarget)
	INTERFACE_PART(XTP_DAYITEM_METRICS, IID_IDatePickerDayMetrics, Dispatch)
END_INTERFACE_MAP()

LPFONTDISP XTP_DAYITEM_METRICS::OleGetFont()
{
	return AxCreateOleFont(&m_fntText, this, (LPFNFONTCHANGED)&XTP_DAYITEM_METRICS::OleSetFont);
}
...
{
	if (ePaintTheme <= xtpCalendarThemeOffice2003)
	{
		SetTheme(new CXTPDatePickerPaintManager());
	}
	else if (ePaintTheme == xtpCalendarThemeOffice2013)
	{
		SetTheme(new CXTPDatePickerThemeOffice2013());
	}
	else if (ePaintTheme == xtpCalendarThemeResource)
	{
		SetTheme(new CXTPDatePickerThemeOffice2007());
	}
	else
