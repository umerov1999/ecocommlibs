...
CXTPCalendarViewEventSubjectEditorColored::CXTPCalendarViewEventSubjectEditorColored(CXTPCalendarControl* pOwner,
...
//<<>> RELEASE MODE FIX >>
	...
	else if (pControl->GetPaintTheme() == xtpCalendarThemeOffice2013)
	{
		CXTPCalendarEventLabel* ptrLabel = GetEvent()->GetLabel();
		COLORREF crBackground = ptrLabel ? ptrLabel->m_clrColor : RGB(255, 255, 255);

		m_pSubjectEditor = new CXTPCalendarViewEventSubjectEditorColored(pControl, this, crBackground);

		if (GetEvent()->IsAllDayEvent() || IsMultidayEvent())
		{
			m_rcSubjectEditor.left += 4;
		}
	}
	else
	{
...
// {BFED5027-B4A5-492f-ADA0-56E1B0772A53}
static const GUID IID_ICalendarViewEvent =
{ 0xbfed5027, 0xb4a5, 0x492f, { 0xad, 0xa0, 0x56, 0xe1, 0xb0, 0x77, 0x2a, 0x53 } };


BEGIN_INTERFACE_MAP(CXTPCalendarViewEvent, CXTPCalendarWMHandler)
	INTERFACE_PART(CXTPCalendarViewEvent, IID_ICalendarViewEvent, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarViewEvent, IID_ICalendarViewEvent)
...
BEGIN_DISPATCH_MAP(CXTPCalendarViewEvents, CCmdTarget)
...
// {550AF03D-69D9-4b45-A487-4F74FC46CC65}
static const GUID IID_ICalendarViewEvents =
{ 0x550af03d, 0x69d9, 0x4b45, { 0xa4, 0x87, 0x4f, 0x74, 0xfc, 0x46, 0xcc, 0x65 } };


BEGIN_INTERFACE_MAP(CXTPCalendarViewEvents, CCmdTarget)
	INTERFACE_PART(CXTPCalendarViewEvents, IID_ICalendarViewEvents, Dispatch)
	//INTERFACE_PART(CXTPCalendarViewEvents, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarViewEvents, IID_ICalendarViewEvents)
