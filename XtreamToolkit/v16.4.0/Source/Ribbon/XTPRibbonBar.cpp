...
void CXTPRibbonBar::EnableFrameTheme(BOOL bEnable)
{
	...
	{
		//Hide standard menu bar when use RibbonBar
		GetCommandBars()->GetSite()->SetMenu(0);
...
int CXTPRibbonBar::CalcGroupsHeight(int nClientHeight)
{
	...
	return nClientHeight + nCaptionHeight + 10;
}
...
void CXTPRibbonBar::Reposition(int cx, int cy)
{
	...
	if (bShowCaption)
	{
		...
		//nFrameBorder = GetRibbonTopBorder();
	...
	else
	{
		m_rcQuickAccess.SetRect(
			3 + nCorner,
			m_rcCaption.top + nFrameBorder - 2,
			m_rcCaption.right,
			m_rcCaption.top + nFrameBorder + nQuickAccessHeight - 2);
		...
		{
			m_rcQuickAccess.top -= m_nQuickAccessHeightDiff;
			m_rcQuickAccess.bottom -= m_nQuickAccessHeightDiff;
		}
	}
...
void CXTPRibbonBar::AddCaptionButton(int nId, BOOL bAdd, BOOL bEnabled, CRect& rcCaption)
{
	...
	if (!pButton && bAdd)
	{
		pButton = m_pControls->Add(new CXTPRibbonBarControlCaptionButton(pHook), nId, NULL, -1, TRUE);
	...
	if (bAdd)
	{
		CSize szButton = pFramePaintManager->GetCaptionButtonSize(nId);
...
// {ADD113E7-46A4-4f99-83D8-C6D3C2B390FC}
static const IID IID_IRibbonBar =
{ 0xADD113e7, 0x46a4, 0x4f99, { 0x83, 0xd8, 0xc6, 0xd3, 0xc2, 0xb3, 0x90, 0xfc } };


IMPLEMENT_OLETYPELIB_EX(CXTPRibbonBar, IID_IRibbonBar)


BEGIN_INTERFACE_MAP(CXTPRibbonBar, CXTPMenuBar)
	INTERFACE_PART(CXTPRibbonBar, IID_IRibbonBar, Dispatch)
...
{
	CXTPTabManagerItem* pItem = (CXTPTabManagerItem*)CCmdTarget::FromIDispatch(lpTab);
