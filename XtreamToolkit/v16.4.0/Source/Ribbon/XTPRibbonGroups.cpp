...
void CXTPRibbonGroup::OnAdjustBorders(int nWidth, CRect rcBorder)
{
	...
	for (i = 0; i < nControlCount; i++)
	{
		...
		if (bCenterItems && !bFirstItem)
		{
			...
			{
				CenterColumn(nFirstItem, i - 1, nGroupClientHeight - nGroupCaptionHeight - 3);
	...
	if (bCenterItems && nControlCount > 0) CenterColumn(nFirstItem, m_pLayoutInfo->nControlCount - 1, nGroupClientHeight - nGroupCaptionHeight - 3);
...
BEGIN_DISPATCH_MAP(CXTPRibbonGroups, CCmdTarget)
...
// {A11113E7-46A4-4f99-83D8-C6D3C2B390FC}
static const IID IID_IRibbonGroups =
{ 0xA11113e7, 0x46a4, 0x4f99, { 0x83, 0xd8, 0xc6, 0xd3, 0xc2, 0xb3, 0x90, 0xfc } };


IMPLEMENT_OLETYPELIB_EX(CXTPRibbonGroups, IID_IRibbonGroups)


BEGIN_INTERFACE_MAP(CXTPRibbonGroups, CCmdTarget)
	INTERFACE_PART(CXTPRibbonGroups, IID_IRibbonGroups, Dispatch)
