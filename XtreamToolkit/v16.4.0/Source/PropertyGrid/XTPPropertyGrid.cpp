...
CXTPPropertyGrid::CXTPPropertyGrid()
{
	...
	lfIcon.lfWeight = FW_NORMAL;
	VERIFY(m_pPaintManager->GetItemMetrics()->m_fontUnderline.CreateFontIndirect(&lfIcon));
...
void CXTPPropertyGrid::Reposition(int cx, int cy)
{
	...
	int nMinViewHeight = 20 + (m_bToolBarVisible ? TOOLBAR_HEIGHT : 0);

	if (m_bHelpVisible)
	{
		if (cy > nMinViewHeight && cy - m_nHelpHeight - SPLITTER_HEIGHT < nMinViewHeight && m_bVariableHelpHeight)
		{
			m_nHelpHeight = max(0, cy - SPLITTER_HEIGHT - nMinViewHeight);
		}
	...
	if (m_bToolBarVisible)
	{
		...
		CRect rcToolBar(0, 0, cx, TOOLBAR_HEIGHT);
	...
	if (!m_bViewVisible)
	{
		if (m_bVerbsVisible) m_nVerbsHeight += rcView.Height() + SPLITTER_HEIGHT;
...
void CXTPPropertyGrid::CreateToolbar()
{
	...
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadToolBar(&m_wndToolbar, XTP_IDR_PROPERTYGRID_TOOLBAR));
}
...
void CXTPPropertyGrid::SetFont(CFont* pFont)
{
	...
	if (pFont->GetLogFont(&lf))
	{
		...
		pMetrics->m_fontUnderline.DeleteObject();
		...
		lf.lfUnderline = 1;
		lf.lfWeight = FW_NORMAL;
		VERIFY(pMetrics->m_fontUnderline.CreateFontIndirect(&lf));
...
void CXTPPropertyGrid::SetTheme(XTPPropertyGridPaintTheme paintTheme)
{
	...
	else if (paintTheme == xtpGridThemeOffice2013)
	{
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridUpdateContext, CCmdTarget)
END_DISPATCH_MAP()

// {53349F02-30A2-44EC-80AB-2ED11DD3D998}
static const IID IID_IPropertyGridUpdateContext =
{ 0x53349f02, 0x30a2, 0x44ec, { 0x80, 0xab, 0x2e, 0xd1, 0x1d, 0xd3, 0xd9, 0x98 } };

BEGIN_INTERFACE_MAP(CXTPPropertyGridUpdateContext, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridUpdateContext, IID_IPropertyGridUpdateContext, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridUpdateContext, IID_IPropertyGridUpdateContext)


BEGIN_DISPATCH_MAP(CXTPPropertyGridVerb, CCmdTarget)
...
// {23349F02-30A2-44EC-80AB-2ED11DD3D998}
static const IID IID_IPropertyGridVerb =
{ 0x23349f02, 0x30a2, 0x44ec, { 0x80, 0xab, 0x2e, 0xd1, 0x1d, 0xd3, 0xd9, 0x98 } };

BEGIN_INTERFACE_MAP(CXTPPropertyGridVerb, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridVerb, IID_IPropertyGridVerb, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridVerb, IID_IPropertyGridVerb)



BEGIN_DISPATCH_MAP(CXTPPropertyGridVerbs, CCmdTarget)
...
// {13349F02-30A2-44EC-80AB-2ED11DD3D998}
static const IID IID_IPropertyGridVerbs =
{ 0x13349f02, 0x30a2, 0x44ec, { 0x80, 0xab, 0x2e, 0xd1, 0x1d, 0xd3, 0xd9, 0x98 } };

BEGIN_INTERFACE_MAP(CXTPPropertyGridVerbs, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridVerbs, IID_IPropertyGridVerbs, Dispatch)
	//INTERFACE_PART(CXTPPropertyGridVerbs, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridVerbs, IID_IPropertyGridVerbs)
